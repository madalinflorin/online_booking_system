...
function APIResponseErrorMessage(props) {
    ...
    const { errorStatus, error } = props;

    const toggleForm = () => {
        setCollapseForm(prevState => !prevState);
    }
    return (
        <div>
            ...
            {errorStatus > 1 &&
                <Modal isOpen={collapseForm} toggle={toggleForm}>
                    <ModalHeader toggle={toggleForm} className={styles.errorTitle}> Server side error informationa: </ModalHeader>
                    <ModalBody>
                        <Row>
